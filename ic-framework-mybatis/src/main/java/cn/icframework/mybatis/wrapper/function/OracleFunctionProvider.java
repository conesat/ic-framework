...
public class OracleFunctionProvider implements SqlFunctionProvider {
    ...
    public QueryField<?> substring(String field, int pos, int len) {
        QueryField<?> queryField = new QueryField<>();
        queryField.setFunc(String.format("SUBSTR(%s, %d, %d)", field, pos, len));
        return queryField;
    }
    ...
    public QueryField<?> anyValue(String field) {
        // Oracle 没有 ANY_VALUE，可以用 MIN(field) 或 MAX(field) 替代
        QueryField<?> queryField = new QueryField<>();
        queryField.setFunc(String.format("MIN(%s)", field));
        return queryField;
    }
    public QueryField<?> concat(Object... vals) {
        // Oracle 的 concat 只能两个参数，可以用 || 拼接
